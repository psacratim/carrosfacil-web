...
$sql = "
SELECT veiculo.id, modelo.nome 'nome_modelo', veiculo.tem_desconto, veiculo.desconto, veiculo.ano, veiculo.kms_rodado, veiculo.tipo_combustivel, veiculo.tipo_cambio, veiculo.preco_venda, veiculo.preco_desconto, veiculo.foto, veiculo.status, veiculo.descricao, marca.nome FROM veiculo INNER JOIN modelo ON modelo.id = veiculo.id_modelo
...
if ($brand !== "") {
    $conditions[] = "marca.nome LIKE '%$brand%'";
}
if ($fuel !== "") {
...
if ($mileage !== "") {
    $conditions[] = "veiculo.kms_rodado BETWEEN 0 AND $mileage";
}
if ($price_range !== "") {
